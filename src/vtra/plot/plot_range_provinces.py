...
def main():
    ...
    for m in range(len(modes)):
        ...
        plot_ranges(flow_file[['min_netrev','max_netrev']],1000, "Percentile rank (%)",
                "Net Revenue ('000 USD/day)","Range of Net revenue flows on links",modes_colors[m],'Net Revenue',plt_file_path)
        ...
        plot_ranges(flow_file[['min_econ_impact','max_econ_impact']],1000,
                ...
                modes_colors[m],'Economic loss',plt_file_path)
        ...
        plot_many_ranges_subplots(fail_dfs,1000, "Percentile rank (%)",
                "EAEL ('000 USD)",
                "{} - Range of Expected Annual economic losses due to link failures".format(modes[m]),
        ...
        for c in range(len(adapt_groups)):
            ...
            plt_file_path = os.path.join(config['paths']['figures'],'{}-{}-ranges-fixed-paramters.png'.format(modes[m],adapt_names[c]))
            plot_ranges(fail_all,adapt_divisor[c], "Percentile rank (%)".format(adapt_labels[c]),
                    "{} ({})".format(adapt_labels[c],adapt_units[c]),"{} - Range of {} of adaptation".format(modes[m],adapt_labels[c]),modes_colors[m],adapt_labels[c],plt_file_path)
