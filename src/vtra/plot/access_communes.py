...
def main():
    ...
    sectors = ['nongnghiep', 'khaikhoang', 'chebien', 'detmay',
               'gogiay', 'sanxuat', 'xaydung', 'thuongmai', 'dichvu']
    sectors_eng = ['agriculture', 'mining', 'processing', 'textile',
                   'wood & paper', 'manufacture', 'construction', 'trade', 'service']
    ...
    for rp in rps:
        ...
        if rp != 'no_flood':
            ...
            create_pie_plots(rp, communes_affected, sectors,
                             sectors_eng, rp_names, figure_path)

            # create bar plot of affected firms and employees in province
            create_bar_plots(rp, communes_affected, sectors,
                             sectors_eng, rp_names, figure_path)
...
def map_distance_per_commune(rp, communes_affected, figure_path, rp_names, data_path):
    ...
    communes_affected['binned_{}'.format(rp)] = pd.cut(
        communes_affected[rp], bins=bins, labels=[0, 1, 2, 3, 4, 5, 6])
    ...
    ax.set_extent([tot_bounds[0]-0.1, tot_bounds[2]+0.1, tot_bounds[1] -
                   0.1, tot_bounds[3]+0.1], crs=proj_lat_lon)
...
#    scale_bar(ax, location=(0.8, 0.05))
    ...
    # create legend
    handles = []
    lnames = ['0-1 km', '1-5 km', '5-10 km', '10-25 km',
              '25-50 km', '50-75 km', 'No Access']  # ,'50-75 km',
    l = 0
    ...
    # set figure title
    plt.title('Distance to major towns in Thanh Hoa for %s' %
              rp_names[rp], fontweight='bold', fontsize=16)
...
def create_pie_plots(rp, communes_affected, sectors, sectors_eng, rp_names, figure_path):
    ...
    only_communes_affected = communes_affected.loc[communes_affected['no_flood']
                                                   != communes_affected[rp]]
    only_communes_affected = only_communes_affected.copy()
    only_communes_affected[sectors] = only_communes_affected[sectors].multiply(
        only_communes_affected['nfirm'], axis='index')
    ...
    cmaplist = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3',
                '#ff7f00', '#ffff33', '#a65628', '#f781bf', '#999999']
    cmap = cmap.from_list('Custom cmap', cmaplist, len(cmaplist))

    # plot figure
    firms_affected.plot(kind='pie', ax=ax2, fontsize=12,
                        cmap=cmap, startangle=90, pctdistance=0.85)

    ax2.axis('equal')
    plt.axis('off')

    # set figure title
    plt.title('Relative distribution of industries affected in \n affected communes Thanh Hoa for %s' %
              rp_names[rp], fontweight='bold', fontsize=17)

    # save figure
    figure_out = os.path.join(
        figure_path, 'Share_industries_affected_communes_Thanh_Hoa_%s.png' % rp)
    plt.savefig(figure_out, dpi=600, bbox_inches='tight')
    ...
    only_communes_affected[sectors] = only_communes_affected[sectors].multiply(
        only_communes_affected['labor'], axis='index')
    firms_affected = only_communes_affected[sectors].sum()
    firms_affected.index = sectors_eng
    ...
    # save figure
    figure_out = os.path.join(
        figure_path, 'Share_employees_affected_communes_Thanh_Hoa_%s.png' % rp)
    plt.savefig(figure_out, dpi=600, bbox_inches='tight')
...
def create_bar_plots(rp, communes_affected, sectors, sectors_eng, rp_names, figure_path):
    ...
    only_communes_affected = communes_affected.loc[communes_affected['no_flood']
                                                   != communes_affected[rp]]
    firms_affected = only_communes_affected[sectors].multiply(
        only_communes_affected['nfirm'], axis='index').sum()

    firms_affected_TH = communes_affected[sectors].multiply(
        communes_affected['nfirm'], axis='index').sum()
