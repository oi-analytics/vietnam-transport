...
def plot_boxplots(input_data, input_labels, input_colors, x_label, y_label, plot_title, plot_file_path):
    ...
    bp = ax.boxplot(input_data, meanline=True, labels=input_labels,
    ...
    for flier, color in zip(bp['fliers'], input_colors):
        flier.set(marker='o', markersize=2, markerfacecolor=color,
...
def plot_boxplots_subplots(scenario_groups, input_data, x_label, y_label, plot_title, plot_file_path):
    ...
    for a in range(len(axs)):
        ...
        bp = axs[a].boxplot(gr_vals, meanline=True, labels=gr_labels,
                            showfliers=True, patch_artist=True)
        ...
        axs[a].text(0.7*len(gr_vals), 1.04*max_input_val, adapt.title(),
...
def main():
    data_path, output_path, figure_path = load_config()['paths']['data'], load_config()[
        'paths']['output'], load_config()['paths']['figures']
    ...
    val_cols = ['adapt_strategy', 'min_daily_loss_2016', 'max_daily_loss_2016', 'min_initial_cost', 'max_initial_cost', 'min_benefit_npv',
                'max_benefit_npv', 'min_cost_npv', 'max_cost_npv', 'min_adapt_npv', 'max_adapt_npv', 'min_bc_ratio', 'max_bc_ratio']
    var_cols = ['daily_loss_2016', 'initial_cost',
                'benefit_npv', 'cost_npv', 'adapt_npv', 'bc_ratio']
    ...
    hazard_df = pd.read_excel(os.path.join(data_path, 'Hazard_data',
                                           'hazard_summary.xlsx'), sheet_name='model_nos')
    # hazard_df = hazard_df[hazard_df['model_no'].isin(['m{}'.format(x) for x in range(16, 28)])]
    hazard_df = hazard_df[hazard_df['model_no'].isin(
        ['m1'] + ['m{}'.format(x) for x in range(3, 12)])]
    scenarios = list(h for h in hazard_df.itertuples(index=False))
    scenario_groups = list(
        set(list(zip(hazard_df['group_no'].values.tolist(), hazard_df['color'].values.tolist()))))
    ...
    for prn in range(0, 1):
        ...
        for g in range(1, 2):
            ...
            for dur in range(0, 1):
                adapt_df = pd.read_excel(
                    adapt_output_excel, sheet_name=grth[1] + '_' + str(duration_max[dur]))
                ...
                for adapt in adapt_strategy:
                    ...
                    for s in range(len(scenarios)):
                        sc = scenarios[s]
                        sc_df = loss_df[(loss_df.hazard_type == sc.hazard_type) & (loss_df.model == sc.model) & (
                            loss_df.climate_scenario == sc.climate_scenario) & (loss_df.year == sc.year) & (loss_df.adapt_strategy == adapt)]
                        # vals_list = []
                        if len(sc_df.index) > 0:
                            for var in var_dict:
                                div = var['divisor']
                                v = var['variable']
                                vals = sc_df['min_{}'.format(v)].append(
                                    sc_df['max_{}'.format(v)])
                                vals = vals.unique()
                ...
                for var in var_dict:
                    ...
                    fig_path = os.path.join(figure_path, '{0}_hazard_{1}_all_options_landslide_flashflood.png'.format(
                        province_name, var['variable']))
                    plot_boxplots_subplots(
                        scenario_groups, adapt_dict[var['variable']], var['x_label'], var['y_label'], province + ': ' + var['title'], fig_path)
