...
def main():
    ...
    rail_edge_file = os.path.join(
        ...
        'weighted_edges_flows_national_rail.shp')
    roads_edge_file = os.path.join(
        config['paths']['data'], 'Results', 'Flow_shapefiles',
        'weighted_edges_flows_national_road.shp')
    coastal_edge_file = os.path.join(
        config['paths']['data'], 'Results', 'Flow_shapefiles',
        'weighted_edges_flows_national_coastal.shp')
    multimodal_edge_file = os.path.join(
        config['paths']['data'], 'Multi', 'multi_edges', 'multimodal_edges.shp')
    multimodal_node_file = os.path.join(
        config['paths']['data'], 'Multi', 'multi_edges', 'multimodal_nodes.shp')

    color_by_type = {'Rail link and station': '#006d2c', 'Road link and junction': '#000000',
                     'Waterway link and port': '#045a8d', 'Multi-Modal link': '#8d3704'}
    ...
    ax.scatter(geoms['roads']['xs'], geoms['roads']['ys'],
               transform=proj_lat_lon, facecolor='#000000', s=25, zorder=5)
    ax.scatter(geoms['railways']['xs'], geoms['railways']['ys'],
               transform=proj_lat_lon, facecolor='#006d2c', s=25, zorder=5)
    ax.scatter(geoms['waterways']['xs'], geoms['waterways']['ys'],
               transform=proj_lat_lon, facecolor='#045a8d', s=25, zorder=5)
