...
from vtra.utils import (get_axes, plot_basemap,
                        plot_basemap_labels_large_region, save_fig, scale_bar,
                        set_ax_bg)
...
def main():
    ...
    for event in events:
        ...
        outcome_event = pd.concat(pd.read_pickle(os.path.join(
            data_path, 'Results', 'IO_Analysis', 'losses_{}.pkl'.format(event)))[event], axis=1)
        ...
        ctry_list_plot = [e for e in ctry_list if e not in (
            'CHN', 'IND', 'IDN', 'BGD', 'MMR', 'MYS')]

        # load dataframe with countries
        states_filename = os.path.join(
            data_path,    'Global_boundaries',    'ne_10m_admin_0_countries_lakes.shp')
    ...
    #    ax = get_axes(get_bounds(ctry_list_plot, world))
        plot_basemap(ax, config['paths']['data'], focus='VNM',
                     neighbours=ctry_list, plot_regions=False)

        # create bin set
        bins = [-1, 0.5, 1, 2.5, 5, 10, 20, 40]

        outcome_countries = outcome_event.groupby(
            outcome_event.index.get_level_values(0)).sum()

        outcome_countries.reset_index(inplace=True)
        subset_viz = world.loc[world['ADM0_A3'].isin(ctry_list), ['ADM0_A3', 'geometry']].merge(
            outcome_countries, left_on='ADM0_A3', right_on='from_region')

        # bin the data for easier plotting with colorscales
        subset_viz['binned_losses'] = pd.cut(
            subset_viz['total_loss'], bins=bins, labels=[0, 1, 2, 3, 4, 5, 6])

        # create cmap
        cmap = cm.get_cmap('Reds', len(bins))  # Colour map (there are many others)
        cmaplist = ['#fff5f0', '#fee0d2', '#fcbba1',
                    '#fc9272', '#fb6a4a', '#ef3b2c', '#cb181d']
        cmap = cmap.from_list('Custom cmap', cmaplist, len(cmaplist))

        # plot figure
        subset_viz.plot(ax=ax, column='binned_losses', cmap=cmap,
                        zorder=2, edgecolor='white', lw=1)
        ...
        # create legend
        handles = []
        lnames = ['0-0.5 m$', '0.5-1 m$', '1-2.5 m$', '2.5-5 m$',
                  '5-10 m$', '10-20 m$', '20-40 m$']  # ,'50-75 km',
        l = 0
        ...
        output_file = os.path.join(config['paths']['figures'],
                                   'mpof-map-typhoon_{}.png'.format(event))
