...
class io_eora(object):
    ...
    def load_all_data(self, data_path=None):
        ...
        LOAD DATA
        """
        FD_data = pd.read_csv('..\..\input_data\Eora26_%s_bp_FD.txt' %
                              self.year, sep='\t', index_col=False, header=None)/1000
        T_data = pd.read_csv('..\..\input_data\Eora26_%s_bp_T.txt' %
                             self.year, sep='\t', index_col=False, header=None)/1000
        VA_data = pd.read_csv('..\..\input_data\Eora26_%s_bp_VA.txt' %
                              self.year, sep='\t', index_col=False, header=None)/1000

        """
        Add labels to the data from 'load_labels'
        """
        FD_data.index = pd.MultiIndex.from_arrays(
            self.T_labels.values.T).droplevel(0).droplevel(1)
        T_data.index = pd.MultiIndex.from_arrays(
            self.T_labels.values.T).droplevel(0).droplevel(1)
        VA_data.index = pd.MultiIndex.from_arrays(self.VA_labels.values.T).droplevel(0)

        FD_data.columns = pd.MultiIndex.from_arrays(
            self.FD_labels.values.T).droplevel(0).droplevel(1)
        T_data.columns = pd.MultiIndex.from_arrays(
            self.T_labels.values.T).droplevel(0).droplevel(1)
        VA_data.columns = pd.MultiIndex.from_arrays(
            self.T_labels.values.T).droplevel(0).droplevel(1)
    ...
    def prep_data(self, curdir=None):
        ...
        subset_FD = subset_FD.iloc[:,
                                   subset_FD.columns.get_level_values(0).isin(self.countries)]
        subset_FD.index.names = ['CNTRY', 'IND']

        # create value added matrix for the subset
        subset_VA = self.VA_data.iloc[:, self.VA_data.columns.get_level_values(
            0).isin(self.countries)]

        # create aggregated transaction matrix for the rest of the world
        T_row = self.T_data.iloc[~self.T_data.columns.get_level_values(0).isin(
            self.countries), ~self.T_data.index.get_level_values(0).isin(self.countries)]
        ...
        ImpTrade_ROW = self.T_data.iloc[~self.T_data.index.get_level_values(0).isin(
            self.countries), self.T_data.columns.get_level_values(0).isin(self.countries)]
        ...
        ExpTrade_ROW = self.T_data.iloc[self.T_data.index.get_level_values(0).isin(
            self.countries), ~self.T_data.columns.get_level_values(0).isin(self.countries)]
        ...
        FD_row = self.FD_data.iloc[~self.FD_data.index.get_level_values(0).isin(
            self.countries), ~self.FD_data.columns.get_level_values(0).isin(self.countries)]
        ...
        ImpFD_ROW = self.FD_data.iloc[~self.FD_data.index.get_level_values(0).isin(
            self.countries), self.FD_data.columns.get_level_values(0).isin(self.countries)]
        ...
        ExpFD_ROW = self.FD_data.iloc[self.FD_data.index.get_level_values(0).isin(
            self.countries), ~self.FD_data.columns.get_level_values(0).isin(self.countries)]
        ...
        VA_row = self.VA_data.iloc[:, ~ self.VA_data.columns.get_level_values(
            0).isin(self.countries)]
        ...
        df1 = ImpTrade_ROW.reset_index().join(
            T_row, on=['CNTRY', 'IND']).set_index(ImpTrade_ROW.index.names)
        df2 = pd.merge(subset_T.reset_index(), ExpTrade_ROW.reset_index(), on=[
                       'CNTRY', 'IND'], how='inner').set_index(['CNTRY', 'IND'])
        subset_T = pd.concat([df1, df2])

        # combine matrices to have a new final demand matrix
        df1 = ImpFD_ROW.reset_index().join(
            FD_row, on=['CNTRY', 'IND']).set_index(ImpFD_ROW.index.names)
        df2 = pd.merge(subset_FD.reset_index(), ExpFD_ROW.reset_index(), on=[
                       'CNTRY', 'IND'], how='inner').set_index(['CNTRY', 'IND'])
        ...
        self.Z_matrix = {r + k: v for r, kv in subset_T.iterrows()
                         for k, v in kv.to_dict().items()}
        self.A_matrix = {r + k: v for r, kv in self.A.iterrows()
                         for k, v in kv.to_dict().items()}
        self.FinalD = {r + k: v for r, kv in subset_FD.iterrows()
                       for k, v in kv.to_dict().items()}
        self.ValueA = {r + (k, ): v for r, kv in VA_TZAetal.iterrows()
                       for k, v in kv.to_dict().items()}
...
class io_oecd(object):
    ...
    def load_labels(self):
        ...
        if 'csv' in self.file:
            FD_labels = pd.read_excel('..\..\input_data\ICIO_2016_2011_labels.xlsx',
                                      sheetname="labels_FD", names=['reg', 'tfd'], header=None)
            Exp_labels = pd.read_excel('..\..\input_data\ICIO_2016_2011_labels.xlsx',
                                       sheetname="labels_ExpROW", names=['export'], header=None)
            T_labels = pd.read_excel('..\..\input_data\ICIO_2016_2011_labels.xlsx',
                                     sheetname="labels_T", header=None, names=['reg', 'ind', 'reg2'])
            VA_labels = pd.read_excel('..\..\input_data\ICIO_2016_2011_labels.xlsx',
                                      sheetname="labels_VA", names=['ValueA'], header=None)
    ...
    def prep_data(self):
        ...
        self.ValueA = {r + (k, ): v for r, kv in self.VA_data.iterrows()
                       for k, v in kv.to_dict().items()}
        self.ImpROW = {r + ('Import', ): 0 for r, kv in self.VA_data.iterrows()
                       for k, v in kv.to_dict().items()}
        self.ExpROW = {r + (k, ): v for r, kv in self.ExpROW_data.iterrows()
                       for k, v in kv.to_dict().items()}
...
class sut_basic(object):
    ...
    def load_all_data(self):
        ...
        LOAD DATA
        """
        self.Use_data = pd.read_excel(self.file, sheetname="USE",
                                      index_col=[0, 1], header=[0, 1])
        self.Sup_data = pd.read_excel(self.file, sheetname="SUP",
                                      index_col=[0, 1], header=[0, 1])
        self.VA_data = pd.read_excel(self.file, sheetname="VA", index_col=[0, 1], header=[0])
        self.ExpROW_data = pd.read_excel(
            self.file, sheetname="ExpROW", index_col=[0, 1], header=[0])
        self.ImpROW_data = pd.read_excel(
            self.file, sheetname="ImpROW", index_col=[0, 1], header=[0])
    ...
    def prep_data(self):
        ...
        Return all the parts of the dataset to the class again
        """
        self.Use = {r + k: v for r, kv in self.Use_data.iterrows()
                    for k, v in kv.to_dict().items()}
        self.Sup = {r + k: v for r, kv in self.Sup_data.iterrows()
                    for k, v in kv.to_dict().items()}
        self.ValueA = {r + (k, ): v for r, kv in self.VA_data.iterrows()
                       for k, v in kv.to_dict().items()}
        self.ImpROW = {r + (k, ): v for r, kv in self.ImpROW_data.iterrows()
