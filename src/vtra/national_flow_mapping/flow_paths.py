...
def network_od_paths_assembly(points_dataframe, node_dict, graph, vehicle_wt, transport_mode, industry_columns, gdf_edges, save_edges=True, output_path='', excel_writer=''):
    ...
    for iter_, path in save_paths_df.iterrows():
        ...
        gdf_edges.loc[gdf_edges['edge_id'].isin(
            min_path), min_ind_cols] += path[ch_min_ind_cols].values
        gdf_edges.loc[gdf_edges['edge_id'].isin(
            max_path), max_ind_cols] += path[ch_max_ind_cols].values
    ...
    if save_edges == True:
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_flows_national_{0}.shp'.format(transport_mode)))
...
def main():
    ...
    od_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'national_scale_flow_ods.xlsx')

    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'national_scale_flow_paths_roads_90_percent.xlsx')
    ...
    for m in range(len(modes_file_paths)):
        ...
        if modes[m] == 'road':
            ...
            G = network_shapefile_to_network(
        ...
        network_od_paths_assembly_changing_tonnages(
            all_ods, node_dict, G, veh_wt[m], usage_factors[m], modes[m], ind_cols, gdf_edges, save_edges=True, output_path=shp_output_path, excel_writer=excl_wrtr)
