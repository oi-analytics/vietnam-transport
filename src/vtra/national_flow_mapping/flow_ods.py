...
def main():
    ...
    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'national_scale_flow_ods_road.xlsx')
    excl_wrtr = pd.ExcelWriter(flow_output_excel)

    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'national_scale_od_matrix_road.xlsx')
    ...
    communes['province_name'] = communes.geometry.apply(
        lambda x: get_nearest_node(x, sindex_provinces, provinces, 'name_eng'))
    ...
    for m in range(len(modes_file_paths)):
        ...
        nodes['province_name'] = nodes.apply(lambda x: extract_gdf_values_containing_nodes(
            x, sindex_provinces, provinces, 'name_eng'), axis=1)
        nodes['od_id'] = nodes.apply(lambda x: extract_gdf_values_containing_nodes(
            x, sindex_provinces, provinces, 'od_id'), axis=1)
        ...
        if modes[m] == 'road':
            ...
            ), edges_df['from_node'].values.tolist(), edges_df['vehicle_co'].values.tolist()))
            nd_veh_list = []
        ...
        nodes = pd.merge(nodes, nodes_frac[['node_id', 'pop_frac']],
    ...
    for ind in ind_cols:
        ...
        for m in range(len(modes_file_paths)):
            ...
            ), nodes['od_id'].values.tolist(), nodes['pop_frac'].values.tolist()))
    ...
    for file in os.listdir(crop_data_path):
        if file.endswith(".tif") and ('spam_p' in file.lower().strip()):
            ...
            for m in range(len(modes_file_paths)):
                ...
                ), nodes['min_{}'.format(crop_name)].values.tolist(), nodes['max_{}'.format(crop_name)].values.tolist(), nodes['pop_frac'].values.tolist()))
