...
from vtra.transport_network_creation import (add_igraph_generalised_costs_roads,
...
def netrev_od_pairs(start_points, end_points):
    ...
    for iter_, place in start_points.iterrows():
        try:
            ...
            save_paths.append(
                (closest_center, place['NEAREST_G_NODE'], 1.0*place['netrev_agri']/12.0, 1.0*place['netrev_noagri']/12.0))
        except:
            print(iter_)
...
if __name__ == '__main__':
    ...
    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'province_roads_commune_center_flow_ods.xlsx')
    ...
    for prn in range(len(province_list)):
        ...
        commune_center_in = os.path.join(
            data_path, 'Points_of_interest', commune_committe_names)
        ...
        flow_output_excel = os.path.join(
            output_path, 'flow_mapping_paths', '{}_roads_od_flow_growth_communes.xlsx'.format(province_name))
        ...
        for grth in growth_scenarios:
            ...
            for year in range(2017, 2050):
                all_ods['min_croptons_{}'.format(year)] = math.pow(
                    (1+grth[0]/100), year - base_year)*all_ods['min_croptons']
                all_ods['max_croptons_{}'.format(year)] = math.pow(
                    (1+grth[0]/100), year - base_year)*all_ods['max_croptons']
                all_ods['min_netrev_{}'.format(year)] = math.pow(
                    (1+grth[0]/100), year - base_year)*all_ods['min_netrev']
                all_ods['max_netrev_{}'.format(year)] = math.pow(
                    (1+grth[0]/100), year - base_year)*all_ods['max_netrev']
