...
def network_od_paths_assembly(points_dataframe, node_dict, graph, vehicle_wt, region_name, gdf_edges, save_edges=True, output_path='', excel_writer=''):
    ...
    if save_edges == True:
        ...
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_commune_center_flows_{0}_{1}_tons.shp'.format(region_name, int(vehicle_wt))))
...
if __name__ == '__main__':
    ...
    od_output_excel = os.path.join(output_path, 'flow_mapping_paths',
                                   'province_roads_commune_center_flow_ods.xlsx')
    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'province_roads_commune_center_flow_paths.xlsx')
    excl_wrtr = pd.ExcelWriter(flow_output_excel)
    ...
    for prn in range(len(province_list)):
        ...
        for tr_wt in truck_unit_wt:
            ...
            network_od_paths_assembly(all_ods, node_dict, G, tr_wt, province_name, gdf_edges,
