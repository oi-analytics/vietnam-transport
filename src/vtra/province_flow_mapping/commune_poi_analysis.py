...
import numpy as np
import pandas as pd
from vtra.transport_network_creation import (add_igraph_time_costs_province_roads,
                                             province_shapefile_to_network)
from vtra.utils import (count_points_in_polygon, extract_value_from_gdf,
                        gdf_clip, get_nearest_node, load_config)
...
if __name__ == '__main__':
    ...
    district_committe_names = ['district_people_committee_points_thanh_hoa.shp',
                               'district_province_peoples_committee_point_binh_dinh.shp', 'district_people_committee_points_lao_cai.shp']

    shp_output_path = os.path.join(output_path, 'flow_mapping_shapefiles')
    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'province_roads_district_center_flow_paths.xlsx')
    ...
    for prn in range(len(province_list)):
        ...
        prov_pop['netrev'] = prov_pop.geometry.apply(lambda x: extract_value_from_gdf(
            x, commune_sindex, prov_communes, 'netrev_village'))
        ...
        # get updated edges
        edges_updated = netrev_edges(province_name, prov_pop, prov_commune_center,
                                     G, save_edges=True, output_path=shp_output_path, excel_writer=excl_wrtr)
