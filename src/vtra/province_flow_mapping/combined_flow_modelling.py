...
from vtra.utils import (count_points_in_polygon, extract_value_from_gdf,
                        gdf_clip, gdf_geom_clip, get_nearest_node, load_config)
...
def netrev_edges(region_name, start_points, end_points, graph, save_edges=True, output_path='', excel_writer=''):
        ...
        try:
            ...
            if pos0_i != pos1_i:
                path = graph.get_shortest_paths(
                    pos0_i, pos1_i, weights='min_cost', output="epath")
                ...
                path_index += 1
                save_paths.append(('path_{}'.format(path_index), get_od_pair,
                                   get_path, place['netrev'], get_travel_cost, get_dist, get_time))
        except:
            print(iter_)

    save_paths_df = pd.DataFrame(save_paths, columns=[
                                 'path_index', 'od_nodes', 'edge_path', 'netrev', 'travel_cost', 'distance', 'time'])
    ...
    if save_edges == True:
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_district_center_flows_{}.shp'.format(region_name)))
    return gdf_edges
        ...
        try:
            ...
            if pos0_i != pos1_i:
                for t in range(len(cr_tons)):
                    ...
                    path = graph.get_shortest_paths(
                        pos0_i, pos1_i, weights=g_costs[t], output="epath")
    ...
    for iter_, row in points_dataframe.iterrows():
        try:
            ...
            if pos0_i != pos1_i:
                ...
                graph = add_igraph_generalised_costs_province_roads(graph, vh_nums, tons)
                path = graph.get_shortest_paths(
                    pos0_i, pos1_i, weights='min_gcost', output="epath")
                ...
                graph = add_igraph_generalised_costs_province_roads(graph, vh_nums, tons)
                path = graph.get_shortest_paths(
                    pos0_i, pos1_i, weights='max_gcost', output="epath")
...
def network_edges_assembly(points_dataframe, graph, tonnage_col, vehicle_wt, cost_criteria):
    ...
    for iter_, row in points_dataframe.iterrows():
        try:
            ...
            if pos0_i != pos1_i:
                path = graph.get_shortest_paths(
                    pos0_i, pos1_i, weights=cost_criteria, output="epath")
                get_path = [graph.es[n]['edge_id'] for n in path][0]
                get_dist = sum([graph.es[n]['length'] for n in path][0])
                get_min_time = sum([graph.es[n]['min_time'] for n in path][0])
                get_max_time = sum([graph.es[n]['max_time'] for n in path][0])
                get_min_gcost = sum([graph.es[n]['min_gcost'] for n in path][0])
                get_max_gcost = sum([graph.es[n]['max_gcost'] for n in path][0])
                save_paths.append((od_pair, get_path, row['netrev_noagri']+row['max_agrirev'], row['netrev_noagri'] +
                                   row['min_agrirev'], get_dist, get_min_time, get_max_time, get_min_gcost, get_max_gcost))
        except:
            print(iter_)
...
def netrev_od_pairs(start_points, end_points):
    ...
    for iter_, place in start_points.iterrows():
        try:
            ...
            get_od_pair = (place['NEAREST_G_NODE'], closest_center)
            save_paths.append(
                (str(get_od_pair), 1.0*place['netrev_agri']/12.0, 1.0*place['netrev_noagri']/12.0))
        except:
            print(iter_)

    od_pairs_df = pd.DataFrame(
        save_paths, columns=['od_nodes', 'netrev_agri', 'netrev_noagri'])
    od_pairs_df = od_pairs_df.groupby(
        ['od_nodes'])['netrev_agri', 'netrev_noagri'].sum().reset_index()
    ...
    for cost_param in cost_list:
        if cost_param.crop_code in x_cols:
            if cost_param.crop_code == 'rice':
                min_croprev += (1.0*min(rice_prod_dist)*ex_rate*cost_param.est_net_rev *
                                (x[cost_param.crop_code]/cost_param.tot_tons))/12.0
                max_croprev += (1.0*max(rice_prod_dist)*ex_rate*cost_param.est_net_rev *
                                (x[cost_param.crop_code]/cost_param.tot_tons))/12.0
            else:
...
if __name__ == '__main__':
    ...
    flow_output_excel = os.path.join(
        output_path, 'flow_mapping_paths', 'province_roads_district_center_flow_paths_1.xlsx')
    ...
    province_path = os.path.join(data_path, 'Vietnam_boundaries',
                                 'who_boundaries', 'who_provinces.shp')
    ...
    for prn in range(len(province_list)):
        ...
        edges_in = os.path.join(data_path, 'Roads', '{}_roads'.format(
            province_name), 'vietbando_{}_edges.shp'.format(province_name))
        nodes_in = os.path.join(data_path, 'Roads', '{}_roads'.format(
            province_name), 'vietbando_{}_nodes.shp'.format(province_name))

        commune_center_in = os.path.join(
            data_path, 'Points_of_interest', district_committe_names[prn])
        ...
        prov_communes['netrev_village'] = exchange_rate * \
            (prov_communes['netrevenue']*prov_communes['nfirm'])/prov_communes['n_villages']
        # also get the net revenue of the agriculture sector which is called nongnghiep
        prov_communes['netrev_village_agri'] = 1.0/365.0 * \
            (prov_communes['nongnghiep']*prov_communes['netrev_village'])
        ...
        prov_pop['NEAREST_C_CENTER'] = prov_pop.geometry.apply(
            lambda x: get_nearest_node(x, sindex_commune_center, prov_commune_center, 'OBJECTID'))
        ...
        for file in os.listdir(crop_data_path):
            if file.endswith(".tif") and 'spam_p' in file.lower().strip():
                ...
                if len(prov_crop.index) > 0:
                    ...
                    print('Done with crop {0} in province {1}'.format(
                        crop_name, province_name))
        ...
        all_ods['croptons'] = all_ods.apply(lambda x: assign_monthly_tons_crops(
            x, rice_prod_months, all_ods_crop_cols), axis=1)
        ...
        all_ods['croprev'] = all_ods.apply(lambda x: assign_io_rev_costs_crops(
            x, cost_values_df, rice_prod_months, all_ods.columns.values.tolist(), exchange_rate), axis=1)
        ...
        network_od_paths_assembly(all_ods, node_dict, G, truck_unit_wt, province_name,
                                  save_edges=True, output_path=shp_output_path, excel_writer=excl_wrtr)
