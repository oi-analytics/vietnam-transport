...
def shapefile_to_network(edges_in, path_width_table):
    ...
    # get the width of edges
    width_range_list = [tuple(x) for x in pd.read_excel(
        path_width_table, sheet_name='widths').values]

    edges['width'] = edges.width.apply(
        lambda x: assign_assumed_width_to_roads(x, width_range_list))
...
def assign_travel_times_and_variability(speed_attributes, variability_attributes, mode_type, variability_location, mode_attribute, distance):
    ...
    st = [s[2] for s in speed_attributes if s[0].lower().strip(
    ) == mode_type and s[1].lower().strip() == mode_attribute]
    if len(st) > 0:
        travel_time = distance/sum(st)
        vt = [v[3] for v in variability_attributes if v[0]
              == mode_type and v[1] == variability_location]
        # print (vt)
...
def create_network_dictionary(network_dictionary, layer_name, edge_id, from_node_id, to_node_id, edge_speed_attribute, edge_geom_attribute, cursor, connection):
    ...
    for row in read_layer:
        ...
        network_dictionary = assign_network_dictionary(
            network_dictionary, e_id, fn_id, tn_id, lgth, sp)
...
def create_networkx_topology(network_dictionary, tonnage, teus):
    ...
    for e in range(len(edges)):
        generalised_cost = wait_costs[e] + travel_costs[e] + tonnage * \
            price_tons[e] + teus*price_teus[e] + variable_costs[e]
...
def add_igraph_costs(G, tonnage, teus):
    ...
    if teus > 0:
        generalised_cost = np.array(G.es['waiting_cost']) + np.array(
            G.es['travel_cost']) + teus*np.array(G.es['price_teus']) + np.array(G.es['variable_costs'])
    else:
        generalised_cost = np.array(G.es['waiting_cost']) + np.array(
            G.es['travel_cost']) + tonnage*np.array(G.es['price_tons']) + np.array(G.es['variable_costs'])
