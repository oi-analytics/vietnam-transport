...
def net_present_value_old(probability_wt, adaptation_options_dataframe, strategy_parameter,
                          parameter_value_list, min_eal, max_eal, edge_options_dictionary,
                          edge_width=1.0, edge_length=1.0):
    for param_val in parameter_value_list:
        ...
        st_min_benefit = probability_wt*edge_width*edge_length * \
            adaptation_options_dataframe.loc[adaptation_options_dataframe[strategy_parameter]
                                             == param_val, 'min_benefit'].sum() + min_eal
        st_max_benefit = probability_wt*edge_width*edge_length * \
            adaptation_options_dataframe.loc[adaptation_options_dataframe[strategy_parameter]
                                             == param_val, 'max_benefit'].sum() + max_eal

        st_min_cost = edge_width*edge_length * \
            adaptation_options_dataframe.loc[adaptation_options_dataframe[strategy_parameter]
                                             == param_val, 'min_cost'].sum()
        st_max_cost = edge_width*edge_length * \
            adaptation_options_dataframe.loc[adaptation_options_dataframe[strategy_parameter]
...
def main():
    ...
    adaptation_df['min_rehab_benefit'] = adaptation_df['rehab_cost_min'] * \
        adaptation_df['total_discount_ratio']
    adaptation_df['max_rehab_benefit'] = adaptation_df['rehab_cost_max'] * \
        adaptation_df['total_discount_ratio']

    adaptation_df['min_cost'] = adaptation_df['adapt_cost_min'] + \
        adaptation_df['maintain_cost_min']*adaptation_df['min_maintain_discount_ratio']
    adaptation_df['max_cost'] = adaptation_df['adapt_cost_max'] + \
        adaptation_df['maintain_cost_max']*adaptation_df['max_maintain_discount_ratio']
    ...
    #         'max_initial_cost','min_cost','max_cost','min_rehab_benefit','min_rehab_benefit']
    ...
    st_min_ini_cost = adaptation_df.loc[adaptation_df['asset_cond']
                                        == 'paved', 'min_initial_cost'].sum()
    st_max_ini_cost = adaptation_df.loc[adaptation_df['asset_cond']
                                        == 'paved', 'max_initial_cost'].sum()
    ...
    st_min_benefit = adaptation_df.loc[adaptation_df['asset_cond'] == 'unpaved', 'rehab_cost_min'].sum(
    )*total_discount_ratio[0] + adaptation_df.loc[adaptation_df['asset_cond'] == 'paved', 'rehab_cost_min'].sum()*sum(total_discount_ratio[1:])
    st_max_benefit = adaptation_df.loc[adaptation_df['asset_cond'] == 'unpaved', 'rehab_cost_max'].sum(
    )*total_discount_ratio[0] + adaptation_df.loc[adaptation_df['asset_cond'] == 'paved', 'rehab_cost_max'].sum()*sum(total_discount_ratio[1:])

    ad_opt.append(st_desc + [st_min_ini_cost, st_max_ini_cost,
                             st_min_cost, st_max_cost, st_min_benefit, st_max_benefit])
    new_ht = 6
    st_desc = ['dyke building', 'dyke', 'rural', 'sea', new_ht, 'million USD/km']
    st_min_ini_cost = adaptation_df.loc[adaptation_df['height_m'] == 4, 'min_initial_cost'].sum(
    ) + (new_ht - 4)*adaptation_df.loc[adaptation_df['height_m'] == 4, 'height_incr_min'].sum()
    st_max_ini_cost = adaptation_df.loc[adaptation_df['height_m'] == 4, 'max_initial_cost'].sum(
    ) + (new_ht - 4)*adaptation_df.loc[adaptation_df['height_m'] == 4, 'height_incr_max'].sum()

    st_min_cost = adaptation_df.loc[adaptation_df['height_m'] == 4, 'min_cost'].sum(
    ) + (new_ht - 4)*adaptation_df.loc[adaptation_df['height_m'] == 4, 'height_incr_min'].sum()
    st_max_cost = adaptation_df.loc[adaptation_df['height_m'] == 4, 'max_cost'].sum(
    ) + (new_ht - 4)*adaptation_df.loc[adaptation_df['height_m'] == 4, 'height_incr_max'].sum()

    st_min_benefit = adaptation_df.loc[adaptation_df['height_m']
                                       == 4, 'min_rehab_benefit'].sum()
    st_max_benefit = adaptation_df.loc[adaptation_df['height_m']
                                       == 4, 'max_rehab_benefit'].sum()
    ad_opt.append(st_desc + [st_min_ini_cost, st_max_ini_cost,
                             st_min_cost, st_max_cost, st_min_benefit, st_max_benefit])
