...
def main():
    ...
    index_cols = ['edge_id', 'hazard_type', 'model', 'climate_scenario',
                  'year', 'road_cond', 'asset_type', 'width', 'road_length']
    ...
    for prn in range(0, 1):
        ...
        for tr_wt in truck_unit_wt:
            adapt_output_excel = os.path.join(
                output_path, 'failure_results', 'single_edge_failures_scenarios_{0}_{1}_tons_adapt_options.xlsx'.format(province_name, int(tr_wt)))
            for grth in growth_scenarios:
                for dur in duration_max:
                    scenarios_df = pd.read_excel(
                        adapt_output_excel, sheet_name=grth[1] + '_' + str(dur))
                    ...
                    for adapt in adapt_strategy:
                        ...
                        adapt_df = adapt_df.set_index(
                            ['model', 'climate_scenario', 'year', 'hazard_type'])
                        ...
                        for sc in scenarios:
                            ...
                            sc_df = adapt_df[(adapt_df['model'] == sc[0]) & (adapt_df['climate_scenario'] == sc[1]) & (
                                adapt_df['year'] == sc[2]) & (adapt_df['hazard_type'] == sc[3])]
                        ...
                        fig_path = os.path.join(output_path, 'failure_results', 'Figures', '{0}_{1}_tons_adapt_options_{2}_growth_{3}_days_{4}.png'.format(
                            province_name, int(tr_wt), grth[1], dur, adapt))
                        plt.tight_layout()
