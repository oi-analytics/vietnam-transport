    ...
    for m in range(len(modes)):
        ...
        for perct in percentage:
            ...
            for t in range(len(types)):
                if single_edge == True:
                    file_name = 'single_edge_failures_od_losses_national_{0}_{1}_{2}_percent_disrupt_summarized.csv'.format(modes[m], types[t],int(perct))
                else:
                    file_name = 'multiple_edge_failures_od_losses_national_{0}_{1}_{2}_percent_disrupt_summarized.csv'.format(modes[m], types[t],int(perct))

                df_path = os.path.join(
                    econ_paths_data, file_name)
