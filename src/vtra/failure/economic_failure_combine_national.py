...
def network_failure_assembly(edge_failure_dataframe, transport_mode, gdf_edges, save_edges=True, output_path=''):
    ...
    drop_cols = [c for c in gdf_edges.columns.values.tolist() if (
        'econ' in c) or (c in ('min_loss', 'max_loss'))]
    if drop_cols:
        for d in drop_cols:
    ...
    if save_edges == True:
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_failures_national_{0}_multi_modal_options.shp'.format(transport_mode)))
...
def main():
    ...
    econ_paths_data = os.path.join(
        data_path, 'Results', 'Economic_Failure_Results', 'summarized')
    ...
    for m in range(len(modes)):
        mode_data_file = os.path.join(
            shp_output_path, 'weighted_edges_failures_national_{0}_multi_modal_options.shp'.format(modes[m]))
        ...
        for t in range(len(types)):
            df_path = os.path.join(
                econ_paths_data, 'single_edge_failures_totals_national_{0}_{1}_multi_modal_options_summarized.csv'.format(modes[m], types[t]))
        ...
        network_failure_assembly(
            edge_impact, modes[m], G_df, save_edges=True, output_path=shp_output_path)
