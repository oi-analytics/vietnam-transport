    ...
    for m in range(len(modes_file_paths)):
        ...
        for t in range(len(types)):
            ...
            flow_paths_data = os.path.join(
                output_path, 'flow_mapping_paths', 'national_scale_flow_paths_roads_{}_percent.xlsx'.format(perct))
            ...
            edge_impact_1 = edge_impact.groupby(['edge_id', 'no_access'])[
                select_cols[2:]].sum().reset_index()
            edge_impact_1.rename(columns={'tr_loss': 'tr_loss_1'}, inplace=True)

            flow_paths_data = os.path.join(
                output_path, 'flow_mapping_paths', 'national_scale_flow_paths_roads_{}_percent.xlsx'.format(100-perct))
            flow_df = pd.read_excel(flow_paths_data, sheet_name=modes[m])
            select_cols = ['origin', 'destination', tons_types[t], cost_types[t]]
            flow_df_select = flow_df[select_cols]
        ...
        df_path = os.path.join(output_path, 'failure_results',
                               'single_edge_failures_transport_losses_national_{0}_{1}_shift.csv'.format(modes[m], 100 - perct))
        edge_impact.to_csv(df_path, index=False)
