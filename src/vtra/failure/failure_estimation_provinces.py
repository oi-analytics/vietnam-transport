...
def network_failure_assembly(edge_failure_dataframe, region_name, vehicle_wt, gdf_edges, save_edges=True, output_path=''):
    ...
    if save_edges == True:
        ...
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_commune_center_failures_{0}_{1}_tons.shp'.format(region_name, int(vehicle_wt))))
...
def main():
    ...
    flow_paths_data = os.path.join(output_path, 'flow_mapping_paths',
                                   'province_roads_commune_center_flow_paths.xlsx')
    ...
    for prn in range(len(province_list)):
        ...
        for tr_wt in truck_unit_wt:
            ...
            for t in range(len(types)):
                ...
                for edge in single_ef_list:
                    ef_dict = igraph_scenario_edge_failures(
                        G_df, [edge], flow_df, tr_wt, path_types[t], rev_types[t], tons_types[t], cost_types[t], dist_types[t], time_types[t], vechicle_types[t])
                    if ef_dict:
                        ef_list += ef_dict

                    print('Done with province {0} edge {1} type {2}'.format(
                        province_name, edge, types[t]))
                ...
                df['econ_loss'] = df['no_access']*df['econ_value'] + \
                    (1 - df['no_access'])*df['vehicle_nums']*(df['new_cost'] - df['old_cost'])
                df['tons_loss'] = df['no_access']*df['tons']
                edge_impact = df[['edge_id', 'tons_loss', 'econ_loss']]
                edge_impact = edge_impact.groupby(
                    ['edge_id'])['tons_loss', 'econ_loss'].sum().reset_index()
                edge_impact.rename(columns={'tons_loss': '{}_tons_loss'.format(
                    types[t]), 'econ_loss': '{}_econ_loss'.format(types[t])}, inplace=True)
            ...
            df_path = os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_commune_access_totals_{0}_{1}_tons.csv'.format(province_name, int(tr_wt)))
            edge_impact.to_csv(df_path, index=False)

            # edge_impact = pd.read_csv(df_path).fillna(0)
            network_failure_assembly(edge_impact, province_name, tr_wt,
                                     G_df, save_edges=True, output_path=shp_output_path)
