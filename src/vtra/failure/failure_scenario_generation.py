...
def assign_failed_edge_attributes(x, edge_properties):
    """
    """
    edge_index = [e for e in range(len(edge_properties))
                  if edge_properties[e][0] == x.edge_id][0]
...
def main():
    ...
    for prn in range(0, 1):
        ...
        edge_attr = list(zip(edges['edge_id'].values.tolist(), edges['road_cond'].values.tolist(
        ), edges['asset_type'].values.tolist(), edges['width'].values.tolist()))
        # print (edge_attr)
        ...
        single_edge_path = os.path.join(
            output_path, 'failure_results', 'single_edge_failures_totals_{0}.csv'.format(province_name))
        edge_impacts = pd.read_csv(single_edge_path)
        edge_impacts_attr = list(zip(edge_impacts['edge_id'].values.tolist(
        ), edge_impacts['min_econ_loss'].values.tolist(), edge_impacts['max_econ_loss'].values.tolist()))
        ...
        for e in edge_impacts_attr:
            all_edge_fail_scenarios.loc[all_edge_fail_scenarios['edge_id']
                                        == e[0], 'min_econ_loss'] = e[1]
            all_edge_fail_scenarios.loc[all_edge_fail_scenarios['edge_id']
                                        == e[0], 'max_econ_loss'] = e[2]
        ...
        df_path = os.path.join(output_path, 'hazard_scenarios',
                               'roads_hazard_intersections_{}.csv'.format(province_name))
