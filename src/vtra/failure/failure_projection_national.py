...
def main():
    ...
    for m in range(len(modes)):
        flow_output_excel = os.path.join(
            output_path, 'failure_results', 'single_edge_failures_totals_national_{0}_projections.xlsx'.format(modes[m]))
        excl_wrtr_1 = pd.ExcelWriter(flow_output_excel)
        for grth in growth_scenarios:
            edge_fail_ranges = []
            for t in range(len(types)):
                df_path = os.path.join(output_path, 'failure_shapefiles',
                                       'weighted_edges_failures_national_{}.shp'.format(modes[m]))
                ...
                for year in range(2016, 2050):
                    edge_impact['{0}_loss_{1}'.format(types[t], year)] = math.pow(
                        (1+grth[0]/100), year - base_year)*df['{}_loss'.format(types[t])]
                ...
                print('Done with national {0} {1} with growth rate {2}'.format(
                    modes[m], types[t], grth))
