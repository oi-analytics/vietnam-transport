...
def network_failure_assembly(edge_failure_dataframe, mode_fail, transport_mode, gdf_edges, save_edges=True, output_path=''):
    ...
    for t in range(len(transport_mode)):
        mode_df = gdf_edges[t]
        mode_transfer_df = edge_failure_dataframe[edge_failure_dataframe['edge_id'].str.contains(
            transport_mode[t])]
        ...
        for iter_, row in mode_transfer_df.iterrows():
            # print (row[1:])
            mode_df.loc[mode_df['edge_id'] == row['edge_id'],
                        failure_columns] = row[failure_columns].values

        industry_columns = list(set([f.split('min_')[1]
                                     for f in failure_columns if 'min' in f]))

        for ind in industry_columns:
            mode_df['swap'] = mode_df.apply(lambda x: swap_min_max(
                x, 'min_{}'.format(ind), 'max_{}'.format(ind)), axis=1)
            mode_df[['min_{}'.format(ind), 'max_{}'.format(ind)]
                    ] = mode_df['swap'].apply(pd.Series)
            mode_df.drop('swap', axis=1, inplace=True)

        if save_edges == True:
            mode_df.to_file(os.path.join(
                output_path, 'weighted_edges_failures_national_{0}_transfer_from_{1}_10_shift.shp'.format(transport_mode[t], mode_fail)))
...
def main():
    ...
    modes_file_paths = [('Roads', 'national_roads'), ('Railways',
                                                      'national_rail'), ('Waterways', 'waterways')]
    ...
    for m in range(len(fail_mode)):
        ...
        for t in range(len(types)):
            ...
            df_path = os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_all_path_impacts_national_{0}_{1}_multi_modal_options_10_shift.csv'.format(fail_mode[m], types[t]))
        ...
        edge_impact = pd.merge(edge_impact, edge_fail_ranges[1], how='left', on=[
                               'edge_id']).fillna(0)
        df_path = os.path.join(output_path, 'failure_results',
                               'single_edge_failures_transfers_national_{0}_10_shift.csv'.format(fail_mode[m]))
        # edge_impact = pd.read_csv(df_path)
        edge_impact.to_csv(df_path, index=False)

        network_failure_assembly(edge_impact, fail_mode[m], [
