    ...
    if edge_path_index:
        ...
        for e in edge_path_index:
            ...
            if not origin_node or not destination_node:
                ...
                network_graph = add_igraph_generalised_costs_province_roads(
...
def network_failure_assembly(edge_failure_dataframe, vehicle_wt, transport_mode, gdf_edges, save_edges=True, output_path=''):
    ...
    if save_edges == True:
        gdf_edges.to_file(os.path.join(
            output_path, 'weighted_edges_failures_national_{0}.shp'.format(transport_mode)))
...
def main():
    ...
    flow_paths_data = os.path.join(
        output_path, 'flow_mapping_paths', 'national_scale_flow_paths.xlsx')
    ...
    for m in range(len(modes_file_paths)):
        ...
        for t in range(len(types)):
            ...
            for edge in single_ef_list:
                ef_dict = igraph_scenario_edge_failures(
                    G_df, [edge], flow_df, veh_wt[m], usage_factors[m][0], usage_factors[m][1], path_types[t], cost_types[t], time_types[t])
                if ef_dict:
                    ef_list += ef_dict
            ...
            df.to_csv(os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_all_paths_national_{0}_{1}_2.csv'.format(modes[m], types[t])), index=False)
            ...
            flow_df_select['tr_loss'] = (1 - flow_df_select['no_access'])*flow_df_select[vehicle_types[t]]*(
                flow_df_select['new_cost'] - flow_df_select[cost_types[t]])
            df_path = os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_all_path_impacts_national_{0}_{1}_2.csv'.format(modes[m], types[t]))
            ...
            select_cols = ['edge_id', 'o_region', 'd_region', 'no_access'] + \
                ind_crop_cols + [rice_type[t], tons_types[t]]
            edge_impact = flow_df_select[select_cols]
            edge_impact = edge_impact[edge_impact['no_access'] == 1]
            edge_impact = edge_impact.groupby(['edge_id', 'o_region', 'd_region'])[
                ind_crop_cols + [rice_type[t], tons_types[t]]].sum().reset_index()
            df_path = os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_totals_national_{0}_{1}_2.csv'.format(modes[m], types[t]))
            ...
            edge_impact = edge_impact.groupby(['edge_id', 'o_region', 'd_region'])[
                'tr_loss'].sum().reset_index()
            df_path = os.path.join(output_path, 'failure_results',
                                   'single_edge_failures_tr_loss_national_{0}_{1}_2.csv'.format(modes[m], types[t]))
            edge_impact.to_csv(df_path, index=False)
            ...
            for col_name in [c for c in select_cols if c not in ['edge_id', 'no_access', rice_type[t], tons_types[t]]]:
                edge_impact.rename(
                    columns={col_name: '{}_'.format(types[t])+col_name}, inplace=True)
        ...
        df_path = os.path.join(output_path, 'failure_results',
                               'single_edge_failures_all_losses_national_{0}.csv'.format(modes[m]))
        edge_impact.to_csv(df_path, index=False)
