    ...
    for prn in range(len(province_list)):
        ...
        for tr_wt in truck_unit_wt:
            flow_output_excel = os.path.join(
                output_path, 'failure_results', 'single_edge_failures_commune_access_totals_{0}_{1}_tons_projections.xlsx'.format(province_name, int(tr_wt)))
            excl_wrtr_1 = pd.ExcelWriter(flow_output_excel)
            for grth in growth_scenarios:
                edge_fail_ranges = []
                for t in range(len(types)):
                    df_path = os.path.join(output_path, 'failure_results', 'single_edge_failures_commune_access_all_path_impacts_{0}_{1}_{2}_tons.csv'.format(
                        province_name, types[t], int(tr_wt)))
                    df = pd.read_csv(df_path).fillna(0)
                    edge_impact = df[['edge_id']]
                    # print (edge_impact)
                    cols = []
                    for year in range(2016, 2050):
                        edge_impact['{0}_econ_value_{1}'.format(types[t], year)] = math.pow(
                            (1+grth[0]/100), year - base_year)*df['econ_value']
                        edge_impact['{0}_tons_{1}'.format(types[t], year)] = math.pow(
                            (1+grth[0]/100), year - base_year)*df['tons']
                        edge_impact['{0}_econ_loss_{1}'.format(types[t], year)] = df['no_access']*edge_impact['{0}_econ_value_{1}'.format(types[t], year)] + (
                            1 - df['no_access'])*np.maximum(1, np.ceil(edge_impact['{0}_tons_{1}'.format(types[t], year)]/tr_wt))*(df['new_cost'] - df['old_cost'])

                        cols += ['{0}_econ_value_{1}'.format(types[t], year), '{0}_tons_{1}'.format(
                            types[t], year), '{0}_econ_loss_{1}'.format(types[t], year)]

                    edge_impact = edge_impact.groupby(['edge_id'])[cols].sum().reset_index()
                    edge_fail_ranges.append(edge_impact)

                    print('Done with province {0} {1} tons {2} with growth rate {3}'.format(
                        province_name, tr_wt, types[t], grth))

                edge_impact = edge_fail_ranges[0]
                edge_impact = pd.merge(edge_impact, edge_fail_ranges[1], how='left', on=[
                                       'edge_id']).fillna(0)
                for year in range(2016, 2050):
                    edge_impact['swap'] = edge_impact.apply(lambda x: swap_min_max(
                        x, 'min_econ_value_{0}'.format(year), 'max_econ_value_{0}'.format(year)), axis=1)
                    edge_impact[['min_econ_value_{0}'.format(year), 'max_econ_value_{0}'.format(
                        year)]] = edge_impact['swap'].apply(pd.Series)
                    edge_impact.drop('swap', axis=1, inplace=True)

                    edge_impact['swap'] = edge_impact.apply(lambda x: swap_min_max(
                        x, 'min_tons_{0}'.format(year), 'max_tons_{0}'.format(year)), axis=1)
                    edge_impact[['min_tons_{0}'.format(year), 'max_tons_{0}'.format(
                        year)]] = edge_impact['swap'].apply(pd.Series)
                    edge_impact.drop('swap', axis=1, inplace=True)

                    edge_impact['swap'] = edge_impact.apply(lambda x: swap_min_max(
                        x, 'min_econ_loss_{0}'.format(year), 'max_econ_loss_{0}'.format(year)), axis=1)
                    edge_impact[['min_econ_loss_{0}'.format(year), 'max_econ_loss_{0}'.format(
                        year)]] = edge_impact['swap'].apply(pd.Series)
                    edge_impact.drop('swap', axis=1, inplace=True)
