...
def network_failure_assembly(edge_failure_dataframe,transport_mode,gdf_edges,save_edges = True,output_path =''):
	...
	if save_edges == True:
		gdf_edges.to_file(os.path.join(output_path,'weighted_edges_commune_center_failures_{0}_5_tons.shp'.format(transport_mode)))
...
def main():
	...
	# for prn in range(0,1):
		...
		mode_data_file = os.path.join(shp_output_path,'weighted_edges_commune_center_failures_{0}_5_tons.shp'.format(province_name))
		G_df = gpd.read_file(mode_data_file)

		print (G_df)
		df_path = os.path.join(data_path,'Results','Adaptation_results','single_edge_failures_commune_access_scenarios_{}_5_tons_adapt_options.xlsx'.format(province_name))
		...
		print (df)
		network_failure_assembly(df,province_name,G_df,save_edges = True,output_path =shp_output_path)
